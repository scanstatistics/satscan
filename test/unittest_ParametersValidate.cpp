...
/* Test Suite for the Parameters validation class. */
BOOST_AUTO_TEST_SUITE( parameter_validation_oliviera_suite )

/* Tests parameters validation requires purely spatial analysis with oliviera. */
BOOST_FIXTURE_TEST_CASE( test_oliviera_analysis_type, new_mexico_fixture ) {
    ...
    // should fail since Oliviera only implemented for purely spatial analysis
...
/* Tests parameters validation of oliviera -- oliviera calculation disabled if known of the additional output files are requested. */
BOOST_FIXTURE_TEST_CASE( test_oliviera_additional_output_files, new_mexico_fixture ) {
    // nm data set uses the Poisson model and space-time analysis initially - so set to purely spatial analysis
    _parameters.SetAnalysisType(PURELYSPATIAL);

    // set oliviera parameter to true -- still should validate but validation should be reset of off since
    // we're not requesting either "Location Information" and "Risk Estimates for Each Location" files -- feature disabled otherwise
    _parameters.setCalculateOlivierasF(true);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), true );
    BOOST_CHECK_EQUAL( _parameters.getCalculateOlivierasF(), false );
    // request "Risk Estimates for Each Location" files
    _parameters.SetOutputRelativeRisksAscii(true);
    _parameters.setCalculateOlivierasF(true);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), true );
    BOOST_CHECK_EQUAL( _parameters.getCalculateOlivierasF(), true );
}

/* Tests parameters validation of oliviera -- test number of monte carlo replications and oliviera data sets. */
BOOST_FIXTURE_TEST_CASE( test_oliviera_monte_carlos, new_mexico_fixture ) {
    // nm data set uses the Poisson model and space-time analysis initially - so set to purely spatial analysis
    _parameters.SetAnalysisType(PURELYSPATIAL);
    _parameters.setCalculateOlivierasF(true);
    // request "Risk Estimates for Each Location" files
...
/* Tests parameters validation of oliviera -- test number of monte carlo replications and oliviera data sets. */
BOOST_FIXTURE_TEST_CASE( test_oliviera_num_datasets, new_mexico_fixture ) {
    // nm data set uses the Poisson model and space-time analysis initially - so set to purely spatial analysis
    _parameters.SetAnalysisType(PURELYSPATIAL);
    _parameters.setCalculateOlivierasF(true);
    // request "Risk Estimates for Each Location" files
    ...
    // test number of oliviera data sets -- should require at least 100 or multiple of 100
    _parameters.setNumRequestedOlivieraSets(0);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), false );
    _parameters.setNumRequestedOlivieraSets(9);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), false );
    _parameters.setNumRequestedOlivieraSets(100);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), true );
    _parameters.setNumRequestedOlivieraSets(101);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), false );
    _parameters.setNumRequestedOlivieraSets(200);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), true );
    _parameters.setNumRequestedOlivieraSets(2000);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), true );
    _parameters.setNumRequestedOlivieraSets(10000);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), true );
}

/* Tests parameters validation of oliviera -- test p-value cutoff. */
BOOST_FIXTURE_TEST_CASE( test_oliviera_p_value_cutoff, new_mexico_fixture ) {
    // nm data set uses the Poisson model and space-time analysis initially - so set to purely spatial analysis
    _parameters.SetAnalysisType(PURELYSPATIAL);
    _parameters.setCalculateOlivierasF(true);
    // request "Risk Estimates for Each Location" files
    ...
    _parameters.setOlivieraPvalueCutoff(0.0);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), true );
    _parameters.setOlivieraPvalueCutoff(1.0);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), true );
    _parameters.setOlivieraPvalueCutoff(-0.1);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), false );
    _parameters.setOlivieraPvalueCutoff(1.1);
    BOOST_CHECK_EQUAL( ParametersValidate(_parameters).Validate(_print), false );
}
BOOST_AUTO_TEST_SUITE_END()
