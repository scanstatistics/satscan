...
for arg in "$@"
  do
    if [ $first -eq "1" ]; then
      echo
      echo "Attempting to create symbolic link to SaTScan shared object file." 
      echo
      first=0
    else
      if ldd "$arg" | grep -q 'not found'
      then
...
# Couldn't create link, display lld output to user.
echo
...
for arg in "$@"
  do
    if [ $first -ne "1" ]; then
      echo "Shared object file: ldd $arg"
      ldd "$arg"
    fi
    first=0
  done
