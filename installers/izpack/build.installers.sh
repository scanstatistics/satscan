...
installer_version="/prj/satscan/installers/v.9.5.x"
...
$IzPack/bin/compile $build/satscan/installers/izpack/windows/install_windows.xml -b $installer_version -o $installer_version/install-9_5_windows.jar -k standard
...
rm $installer_version/install-9_5_windows.jar

# prompt user to sign the exe file created by launch4j
echo
echo "Run the Windows batch file 'signWindowsInstaller.bat' now to sign install-9_5_windows.exe. Hit <enter> once done ..."
read dummy

# Build Windows command-line only archive
rm -f $installer_version/satscan.9.5_windows.zip
zip $installer_version/satscan.9.5_windows.zip -j $build/satscan/batch_application/Win32/Release/SaTScanBatch.exe
zip $installer_version/satscan.9.5_windows.zip -j $build/satscan/batch_application/x64/Release/SaTScanBatch64.exe
cd $build/satscan/installers
zip $installer_version/satscan.9.5_windows.zip -j documents/*
zip $installer_version/satscan.9.5_windows.zip sample_data/*

#######   ############ Linux ################################################################################
# Build the IzPack Java installer for Linux.
$IzPack/bin/compile $build/satscan/installers/izpack/linux/install_linux.xml -b $installer_version -o $installer_version/install-9_5_linux.jar -k standard
chmod a+x $installer_version/install-9_5_linux.jar

# Build batch binaries archive for Linux.
rm -f $installer_version/satscan.9.5_linux.tar.bz2
cd $build/binaries/linux
tar -cf $installer_version/satscan.9.5_linux.tar satscan*
cd $build/satscan/installers
tar -rf $installer_version/satscan.9.5_linux.tar documents/*
tar -rf $installer_version/satscan.9.5_linux.tar sample_data/*
bzip2 -f $installer_version/satscan.9.5_linux.tar
...
$IzPack/bin/compile $build/satscan/installers/izpack/mac/install_mac.xml -b $installer_version -o $installer_version/install-9_5_mac.jar -k standard

# Build Mac OS X Application Bundle from IzPack Java Installer
rm -rf $installer_version/install-9_5_mac.zip
rm -rf $build/satscan/installers/izpack/mac/Install.app
python $build/satscan/installers/izpack/mac/izpack2app/izpack2app.py $installer_version/install-9_5_mac.jar $build/satscan/installers/izpack/mac/Install.app
...
zip $installer_version/install-9_5_mac.zip -r ./Install.app/*
rm $installer_version/install-9_5_mac.jar
rm -rf $build/satscan/installers/izpack/mac/Install.app
chmod a+x $installer_version/install-9_5_mac.zip

# Build batch binaries archive for Mac OS X.
rm -f $installer_version/satscan.9.5_mac.tar.bz2
cd $build/binaries/mac
tar -cf $installer_version/satscan.9.5_mac.tar satscan
cd $build/satscan/installers
tar -rf $installer_version/satscan.9.5_mac.tar documents/*
tar -rf $installer_version/satscan.9.5_mac.tar sample_data/*
bzip2 -f $installer_version/satscan.9.5_mac.tar
