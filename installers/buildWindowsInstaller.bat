...
\\oriole-03-int\satscan\build.area\satscan\installers\izpack\sign4j\sign4j.exe --verbose \\oriole-03-int\imsadmin\code.sign.cert.ms.auth.verisign\signtool.exe sign /f \\oriole-03-int\imsadmin\code.sign.cert.ms.auth.verisign\ims-cross-sign.pfx /p "&4L(JyhyOmwF)$Z" /t http://timestamp.verisign.com/scripts/timstamp.dll /v \\oriole-03-int\satscan\installers\v.10.x.0\install-10_0_windows.exe

REM Verify the installer exe file is codesigned correctly.
\\oriole-03-int\imsadmin\code.sign.cert.ms.auth.verisign\signtool.exe verify /pa /v \\oriole-03-int\satscan\installers\v.10.x.0\install-10_0_windows.exe