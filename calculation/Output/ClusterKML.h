...
class BaseClusterKML {
  protected:
      ...
      unsigned int addClusters(
          const MostLikelyClustersContainer& clusters, const SimulationVariables& simVars, unsigned int iteration,
          std::ofstream& outKML, file_collection_t& fileCollection, unsigned int clusterOffset=0
      );
      ...
      void writeCluster(
          file_collection_t& fileCollection, std::ofstream& outKML, const CCluster& cluster, 
          int iCluster, const SimulationVariables& simVars, unsigned int iteration
      ) const;
