...
void stsRunHistoryFile::LogNewHistory(const AnalysisRunner& AnalysisRun) {
   ...
   try {
      ...
      if(!gbSequential && gbPrintPVal && params.GetReportCriticalValues()) {    // only print 0.01 and 0.05 cutoffs if pVals are printed, else this would result in access underrun - AJV
