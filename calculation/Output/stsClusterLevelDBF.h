...
class stsClusterLevelDBF : public DBaseOutput {
   private:
      ...
      void	Setup(const ZdString& sOutputFileName, const int iDimension, const bool bPrintEllipses = false);
   protected:
      virtual void  SetupFields(ZdPointerVector<ZdField>& vFields);
   public:
      __fastcall stsClusterLevelDBF(const long lRunNumber, const int iCoordType, const ZdFileName& sOutputFileName, const int iDimension = 2, const bool bPrintPVal = true, const bool bPrintEllipses = false);
