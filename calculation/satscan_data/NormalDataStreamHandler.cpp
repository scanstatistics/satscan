...
/** constructor */
NormalDataStreamHandler::NormalDataStreamHandler(CSaTScanData & Data, BasePrint * pPrint)
  ...
  try {
    ...
    if (nCount < 0) {//validate that count is not negative or exceeds type precision
      if (strstr(Parser.GetWord(1), "-"))
        ...
        gpPrint->PrintInputWarning("Error: The case count '%s' exceeds the maximum allowed value of %ld in record %ld of %s.\n",
    ...
    if (!Parser.GetWord(3)) {
      gpPrint->PrintInputWarning("Error: Record %d, of the %s, is missing the continuous variable.\n",
                                 Parser.GetReadCount(), gpPrint->GetImpliedFileTypeString().c_str());
      return false;
    }
...
//        gpPrint->PrintInputWarning("Error: Negative continuos variable in record %ld of cases file.\n", Parser.GetReadCount());
//      else
//        gpPrint->PrintInputWarning("Error: Continuos variable '%s' exceeds maximum value of %i in record %ld of population file.\n",
