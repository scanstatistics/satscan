...
const RealDataContainer_t & PoissonDataSetHandler::buildOliveiraDataSets() {
    try {
        ...
        for (unsigned int i=0; i < gDataHub.GetNumDataSets(); ++i) {
            _oliveira_data_sets.push_back(new RealDataSet(gDataHub.GetNumTimeIntervals(), gDataHub.GetNumIdentifiers(), gDataHub.GetNumMetaIdentifiers(), gParameters, i + 1));
...
bool PoissonDataSetHandler::CreatePopulationData(RealDataSet& DataSet) {
  ...
  tract_t                                               t, tNumTracts = gDataHub.getIdentifierInfo().getIdentifiers().size();
  int                                                   iCategoryIndex;
...
/** Reads data from population file into dataset structures. Reading of the
    ...
    Identifiers of population file are matched to identifiers specifed in coordinates file; so this function should not be called before
    the coordinates file has been read. */
bool PoissonDataSetHandler::ReadPopulationFile(RealDataSet& DataSet) {
  int                                       iCategoryIndex;
  bool                                      bValid=true, bEmpty=true;
  tract_t                                   IdentifierIndex;
  float                                     fPopulation;
  ...
  try {
    ...
    while (!gPrint.GetMaximumReadErrorsPrinted() && Source->ReadRecord()) {
		...
		DataSetHandler::RecordStatusType eStatus = RetrieveIdentifierIndex(*Source, IdentifierIndex);
        ...
        auto itr = std::lower_bound(vprPopulationDates.begin(), vprPopulationDates.end(), prPopulationDate, ComparePopulationDates());
    ...
    if (bValid && !bEmpty) {
      ...
      while (!gPrint.GetMaximumReadErrorsPrinted() && Source->ReadRecord()) {
          ...
          DataSet.getPopulationData().AddCovariateCategoryPopulation(IdentifierIndex, iCategoryIndex, prPopulationDate, fPopulation);
          bEmpty = false;
      }
    }
