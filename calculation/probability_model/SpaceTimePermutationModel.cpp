...
/** constructor */
CSpaceTimePermutationModel::CSpaceTimePermutationModel(const CParameters& Parameters, CSaTScanData& DataHub, BasePrint& PrintDirection)
...
void CSpaceTimePermutationModel::CalculateMeasure(RealDataSet& DataSet) {
  int                   i, j, c, iNumCategories(DataSet.GetPopulationData().GetNumCovariateCategories());
    ...
    for (c=0; c < iNumCategories; ++c) {
       ...
       if (Population.GetNumCovariateCategoryCases(c)) {
         for (i=0; i < gDataHub.m_nTimeIntervals; ++i) {
            T_C = ppCases[i][0];
            //Calculate T/C
            for (j=1; j < gDataHub.GetNumTracts(); ++j)
               T_C += ppCases[i][j];
