...
void stsMCSimJobSource::RegisterResult_NoAutoAbort(job_id_type const & rJobID, param_type const & rParam, result_type const & rResult) {
    try {
        ...
        if (uiJobsProcessedCount == 10 || (_frequent_estimations && (uiJobsProcessedCount % SIMULATION_EST_MODULAS == 0))) {
            _frequent_estimations = ::ReportTimeEstimate(gConstructionTime, guiJobCount, rParam, grPrintDirection, true, uiJobsProcessedCount != 10) > FREQUENT_ESTIMATES_SECONDS;
